...
<!-- No whitespace between the tags (see StepRow). "Used by" is rows, not
     prose: clicking one jumps to the step that reads this. Unkeyed: `uses`
     comes over the wire, and a repeated index renders twice, as today, rather
     than throwing. -->
{#if !Object.hasOwn(declared, name)}<div class="hint">That parameter is gone.</div>{:else}<div
  ...
  class="dlabel">Default</div><div class="pairs"><Pair k="default" v={fallback} /></div>{/if}<!-- eslint-disable-next-line svelte/require-each-key -- (an index may repeat, and a repeat must render, not throw) --><div
  class="dlabel">Used by</div>{#if used.length}<div class="rows">{#each used as i}<StepRow
