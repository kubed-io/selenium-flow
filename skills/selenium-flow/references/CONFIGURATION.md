...
Point an MCP client at `/mcp` and everything else at `/browser/*` — **under the
mount** when `ROUTE_PREFIX` is set, so `/flow/mcp` and `/flow/browser/*` for
`ROUTE_PREFIX=/flow`. Only the four ops endpoints also answer at the root.
