...
# Dropped from a step whose value came from somewhere it must not come back
# from. `write` reads the field back and returns it, deliberately, so that a
# caller can confirm the text landed — which for a bound secret would hand the
# value straight back to the model on the very call meant to protect it
# (§F1.25). The read still happens inside the action; what this stops is it
# reaching a report or a log.
...
def _clean(result, guarded: set) -> dict:
    ...
    if guarded:
        for field in SENSITIVE_RESULT_FIELDS:
            if field in cleaned:
                cleaned[field] = None
    return cleaned
...
def run(
    ...
    for number, step in enumerate(document.get("steps") or [], start=1):
        ...
        try:
            result = _clean(method(session_id, **kwargs), guarded)
            entry["ok"] = True
            last = result
