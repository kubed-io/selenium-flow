...
- **`skills/`, `prompts/` and `js/` are mapped into the package** by
  ...
  silently absent from an installed copy. The built UI is the exception: it is
  generated by `npm --prefix ui run build`, gitignored, and collected by a
  `package-data` glob so an install that skipped the build still works.
