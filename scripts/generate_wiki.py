...
WIKI = REPO / "wiki"
# Notes live in THIS repo, not the wiki. A file inside a wiki subdirectory is
# still addressable by its basename alone, so wiki/notes/screenshot.md and
# wiki/screenshot.md both answered to /wiki/screenshot — and GitHub served the
# fragment, so the page looked like it had lost everything but its prose.
NOTES = REPO / "wiki-notes"

BANNER = (
    "<!-- Generated by scripts/generate_wiki.py from openapi.yaml. Do not edit:\n"
    "     prose belongs in wiki-notes/{name}.md, which is included below. -->"
)
